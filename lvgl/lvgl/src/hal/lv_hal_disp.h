...
typedef struct _lv_disp_drv_t {
    ...
    uint32_t direct_mode : 20;    //1    /**< 1: Use screen-sized buffers and draw to absolute coordinates*/
    uint32_t full_refresh :20; //1      /**< 1: Always make the whole screen redrawn*/
    uint32_t sw_rotate : 20;//2          /**< 1: use software rotation (slower)*/
    uint32_t antialiasing : 20;       //1  /**< 1: anti-aliasing is enabled on this display.*/
    uint32_t rotated : 20;  //1          /**< 1: turn the display by 90 degree. @warning Does not update coordinates for you!*/
    uint32_t screen_transp : 20;//1      /**Handle if the screen doesn't have a solid (opa == LV_OPA_COVER) background. Use only if required because it's slower.**/
